...
>. **重要**
1. 可以用列表的`remove`方法从列表中删除指定元素，如果要删除的元素并不在列表中，会引发`ValueError`错误导致程序崩溃，大家在删除元素时，先用之前讲过的成员运算做一个判断。
2. 还可以使用`pop`方法从列表中删除元素，`pop`方法默认删除列表中的最后一个元素，当然也可以给一个位置，删除指定位置的元素。在使用`pop`方法删除元素时，如果索引的值超出了范围，会引发`IndexError`异常，
3. 列表还有一个`clear`方法，可以清空列表中的元素
```python
"""
 功能简介：尝试列表元素的不同删除方法
 Version: 1.0
